...
# +-----------------------------------------------------------------+
# |                   SHAPE RACER GAME (GAME 4)                     |
# +-----------------------------------------------------------------+
class ShapeRacerGame:
    def __init__(self, master, launcher):
        self.master = master
        self.launcher = launcher
        self.master.title("Shape Racer")
        self.width, self.height = 700, 700
        try:
            root_x, root_y = self.master.master.winfo_x(), self.master.master.winfo_y()
            self.master.geometry(f"{self.width}x{self.height + 100}+{root_x + 50}+{root_y + 50}")
        except:
            self.master.geometry(f"{self.width}x{self.height + 100}")
        self.master.resizable(False, False)
        self.master.configure(bg='#333')
        self.master.grab_set()
        ...
        self.good_shapes = {"Circle": "green", "Oval": "blue"}
        self.bad_shapes = {"Triangle": "red", "Star": "yellow"}
    ...
    def setup_splash_screen(self):
        tk.Label(self.splash_frame, text="Shape Racer", font=("Arial", 28, "bold"), fg="white", bg="#333").pack(pady=20)
        instructions = "You have 30 seconds to score as many points as you can!\n\nDrive with the arrow keys.\nCollect circular shapes for +50 points.\nAvoid spiky shapes or lose 25 points!"
        ...
        tk.Button(self.splash_frame, text="Start Game", font=("Arial", 16), command=self.start_game).pack(pady=20)
        self.splash_frame.pack(fill=tk.BOTH, expand=True)

    def setup_game_screen(self):
        self.title_label = tk.Label(self.game_frame, text="Shape Racer", font=("Arial", 28, "bold"), fg="white",
                                    bg="#333");
        self.title_label.pack(pady=5)
        info_frame = tk.Frame(self.game_frame, bg='#333');
        info_frame.pack(pady=5)
        ...
        self.canvas = tk.Canvas(self.game_frame, width=self.width, height=self.height, bg="#567d46",
    ...
    def setup_game_over_screen(self):
        ...
        end_button_frame = tk.Frame(self.game_over_frame, bg="#333");
        end_button_frame.pack(pady=20)
        tk.Button(end_button_frame, text="Play Again", font=("Arial", 16), command=self.start_game).pack(side=tk.LEFT,
                                                                                                         padx=10)
        tk.Button(end_button_frame, text="Back to Main Menu", font=("Arial", 16), command=self.master.destroy).pack(
            side=tk.RIGHT, padx=10)
    ...
    def start_game(self):
        ...
        self.player_shape = self.canvas.create_rectangle(self.player_x - 10, self.player_y - 10, self.player_x + 10,
                                                         self.player_y + 10, fill="cyan")
    ...
    def game_over(self):
        self.is_running = False
        self.launcher.check_high_score("Shape Racer", self.score)
        self.game_frame.pack_forget()
        self.final_score_label.config(text=f"Final Score: {self.score}")
        self.game_over_frame.pack(fill=tk.BOTH, expand=True)
    ...
    def spawn_loop(self):
        if not self.is_running: return
        self.spawn_single_shape()
        self.master.after(1500, self.spawn_loop)

    def spawn_single_shape(self):
        if random.random() > 0.4:  # 60% chance of good shape
            name, color = random.choice(list(self.good_shapes.items()))
            tag = "good"
        else:
            name, color = random.choice(list(self.bad_shapes.items()))
            tag = "bad"

        x, y, size = random.randint(50, self.width - 50), random.randint(50, self.height - 50), 20
        item = None
        if name == "Circle":
            item = self.canvas.create_oval(x, y, x + size, y + size, fill=color, tags=(tag, name))
        elif name == "Oval":
            item = self.canvas.create_oval(x, y, x + size + 10, y + size, fill=color, tags=(tag, name))
        elif name == "Triangle":
            item = self.canvas.create_polygon(x + size / 2, y, x, y + size, x + size, y + size, fill=color,
                                              tags=(tag, name))
        elif name == "Star":
            item = self.canvas.create_polygon(x + 12, y, x + 15, y + 8, x + 25, y + 9, x + 17, y + 15, x + 20, y + 25,
                                              x + 12, y + 18, x + 4, y + 25, x + 7, y + 15, x, y + 9, x + 9, y + 8,
                                              fill=color, tags=(tag, name))

        if item:
            self.shapes[item] = {'dx': random.choice([-1, 1]), 'dy': random.choice([-1, 1])}
    ...
    def game_loop(self):
        ...
        # Player movement
        if "Left" in self.keys_pressed: self.player_x -= self.player_speed
        if "Right" in self.keys_pressed: self.player_x += self.player_speed
        if "Up" in self.keys_pressed: self.player_y -= self.player_speed
        if "Down" in self.keys_pressed: self.player_y += self.player_speed

        self.player_x = max(10, min(self.player_x, self.width - 10))
        self.player_y = max(10, min(self.player_y, self.height - 10))
        self.canvas.coords(self.player_shape, self.player_x - 10, self.player_y - 10, self.player_x + 10,
                           self.player_y + 10)

        # Shape movement and bouncing
        for shape_id, properties in list(self.shapes.items()):
            self.canvas.move(shape_id, properties['dx'], properties['dy'])
            x1, y1, x2, y2 = self.canvas.coords(shape_id)
            if x1 <= 0 or x2 >= self.width: properties['dx'] *= -1
            if y1 <= 0 or y2 >= self.height: properties['dy'] *= -1

        self.check_collisions()
        self.master.after(20, self.game_loop)

    def check_collisions(self):
        px1, py1, px2, py2 = self.canvas.coords(self.player_shape)
        overlapping_items = self.canvas.find_overlapping(px1, py1, px2, py2)

        for shape_id in overlapping_items:
            if shape_id != self.player_shape and shape_id in self.shapes:
                tags = self.canvas.gettags(shape_id)
                if "good" in tags:
                    self.score += 50
                    self.feedback_label.config(text="Good shape! +50", fg="lightgreen")
                elif "bad" in tags:
                    self.score -= 25
                    self.feedback_label.config(text="Ouch! Spiky! -25", fg="orange")
                self.score_label.config(text=f"Score: {self.score}")
