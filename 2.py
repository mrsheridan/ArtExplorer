...
class ValueMatcherGame:
    ...
    def setup_splash_screen(self):
        tk.Label(self.splash_frame, text="Value: The Lightness or Darkness of a Color", font=("Arial", 18, "bold"),
                 fg="white", bg="#333", wraplength=550).pack(pady=20)

        value_scale_frame = tk.Frame(self.splash_frame, bg="#333")
        scale_canvas = tk.Canvas(value_scale_frame, width=400, height=40, bg="#333", highlightthickness=0)
        for i in range(10):
            gray_val = int(255 * ((9 - i) / 9))
            hex_code = f'#{gray_val:02x}{gray_val:02x}{gray_val:02x}'
            scale_canvas.create_rectangle(i * 40, 0, (i + 1) * 40, 40, fill=hex_code, outline="")
        scale_canvas.pack()
        tk.Label(value_scale_frame, text="100% White to 100% Black", font=("Arial", 10), fg="white", bg="#333").pack()
        value_scale_frame.pack(pady=10)

        instructions = (
            "This game has 10 rounds, each lasting 10 seconds.\n"
            "Score up to 100 points per round based on accuracy.\n\n"
            "Use the ARROW KEYS to change your value.\n"
            "Press SPACEBAR to lock in your choice."
        )
        tk.Label(self.splash_frame, text=instructions, font=("Arial", 14), justify=tk.CENTER, fg="white",
                 bg="#333").pack(pady=20)
        tk.Button(self.splash_frame, text="Start Game", font=("Arial", 16), command=self.start_game).pack(pady=10)
